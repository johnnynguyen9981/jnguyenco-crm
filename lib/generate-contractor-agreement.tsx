...
const EN: Strings = {
  ...
  sub12: "1.2 Equipment",
  body12: "Company will provide equipment reasonably required to perform the Services, including camera bodies, lenses, and memory cards. Contractor is equally welcome to use their own equipment instead of, or alongside, equipment provided by Company, provided all output meets Company's quality standards. Any equipment provided by Company remains the property of Company and must be returned in good working condition at the end of each shoot day (or the engagement, as applicable). All raw image and video files captured during a shoot must be handed over to Company by the end of that shoot day.",
...
const VI: Strings = {
  ...
  sub12: "1.2 Thiết bị",
  body12: "Công ty sẽ cung cấp các thiết bị cần thiết hợp lý để thực hiện Dịch vụ, bao gồm thân máy ảnh, ống kính, và thẻ nhớ. Nhà thầu cũng có thể sử dụng thiết bị cá nhân của mình thay cho, hoặc cùng với, thiết bị do Công ty cung cấp, miễn là sản phẩm đầu ra đáp ứng tiêu chuẩn chất lượng của Công ty. Mọi thiết bị do Công ty cung cấp vẫn thuộc quyền sở hữu của Công ty và phải được hoàn trả trong tình trạng hoạt động tốt vào cuối mỗi ngày quay/chụp (hoặc khi kết thúc hợp tác, tùy trường hợp). Toàn bộ tệp hình ảnh và video gốc (raw) ghi nhận trong ngày quay/chụp phải được bàn giao cho Công ty ngay khi kết thúc ngày quay/chụp đó.",
...
const AgreementPage = ({ lang, d, signatureDataUri, showLangTag, bilingualNotice }: AgreementPageProps) => {
  ...
  return (
    <Page size="A4" style={s.page}>
      ...
      {(d.role === "PHOTOGRAPHER" || d.role === "BOTH") && (
        <>
          <Text style={s.subheading}>{t.sub12}</Text>
          <Text style={s.body}>{t.body12}</Text>
        </>
      )}
